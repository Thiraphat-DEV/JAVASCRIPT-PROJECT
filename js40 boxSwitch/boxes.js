...
window.onload = () => {
  ...
  //data object fetch in stackOverflow
  const boxes = [
    {
      name: "Miyah Myles",
      position: "Marketing",
      photo: `https://randomuser.me/api/portraits/women/${randomBoxes()}.jpg`,
    ...
    },
    {
      ...
      photo: `https://randomuser.me/api/portraits/men/${randomBoxes()}.jpg`,
  ...
  function updateBoxes() {
    for (let idx = 0; idx <= boxes.length; idx++) {
      const { name, position, photo, text } = boxes[idx];
      boxs.innerHTML = text;
      userImg.src = photo;
    ...
    boxes.forEach(idx => {
      boxesContainer.append(boxes[idx])
    })
  }
